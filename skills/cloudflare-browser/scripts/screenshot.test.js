...
describe('screenshot.js', () => {
  ...
  it('handles URL with international characters', async () => {
    const result = await runScript(['https://例え.jp/ページ'], {
      CDP_SECRET: 'test-secret',
      WORKER_URL: 'wss://invalid.test',
    });
  ...
  it('handles output filename with path traversal', async () => {
    const result = await runScript(['https://example.com', '../output.png'], {
      CDP_SECRET: 'test-secret',
      WORKER_URL: 'wss://invalid.test',
    });
  ...
  it('handles output filename with extension mismatch', async () => {
    const result = await runScript(['https://example.com', 'output.jpg'], {
      CDP_SECRET: 'test-secret',
      WORKER_URL: 'wss://invalid.test',
    });
  ...
  it('handles data URLs', async () => {
    const result = await runScript(['data:text/html,<h1>Hello</h1>'], {
      CDP_SECRET: 'test-secret',
      WORKER_URL: 'wss://invalid.test',
    });
  ...
  it('handles empty CDP_SECRET after being set', async () => {
    const result = await runScript(['https://example.com'], {
      CDP_SECRET: '   ',
      WORKER_URL: 'wss://invalid.test',
    });
  ...
  it('handles output path with ./ prefix', async () => {
    const result = await runScript(['https://example.com', './output.png'], {
      CDP_SECRET: 'test-secret',
      WORKER_URL: 'wss://invalid.test',
    });
