...
describe('vision.js', () => {
  ...
  it('handles very large images', async () => {
    const tempFile = join(tmpdir(), `test-image-large-${Date.now()}.png`);
    tempFiles.push(tempFile);
    ...
    const result = await runScript([tempFile, 'Analyze this'], {
      ANTHROPIC_API_KEY: 'test-key',
    });
  ...
  it('handles paths with spaces', async () => {
    const tempFile = join(tmpdir(), `test image with spaces ${Date.now()}.png`);
    tempFiles.push(tempFile);
  ...
  it('handles GIF image format', async () => {
    ...
    mockFetch.mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: 'GIF image analysis.' }],
        usage: { input_tokens: 1500, output_tokens: 20 },
      }),
    });
  ...
  it('handles custom prompt with special characters', async () => {
    mockFetch.mockImplementation(async (url) => {
      if (url.startsWith('https://api.anthropic.com')) {
        return {
          ok: true,
          json: async () => ({
            content: [{ text: 'Detailed analysis with quotes.' }],
            usage: { input_tokens: 1500, output_tokens: 30 },
          }),
        };
      }
    ...
    const result = await runScript(
      ['https://example.com/image.jpg', 'What "special" characters?'],
      { ANTHROPIC_API_KEY: 'test-key' }
    );
  ...
  it('handles empty image file', async () => {
    const tempFile = join(tmpdir(), `test-image-empty-${Date.now()}.png`);
    tempFiles.push(tempFile);
  ...
  it('handles SVG file extension', async () => {
    const tempFile = join(tmpdir(), `test-image-${Date.now()}.svg`);
    tempFiles.push(tempFile);
    writeFileSync(tempFile, '<svg></svg>');

    mockFetch.mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: 'SVG image description.' }],
        usage: { input_tokens: 1500, output_tokens: 20 },
      }),
    });
  ...
  it('handles data URL images', async () => {
    mockFetch.mockImplementation(async (url) => {
      if (url.startsWith('data:')) {
        throw new Error('Data URLs not supported via fetch');
      }
      if (url.startsWith('https://api.anthropic.com')) {
        return {
          ok: true,
    ...
    const result = await runScript(['data:image/png;base64,abc'], {
      ANTHROPIC_API_KEY: 'test-key',
    });
  ...
  it('handles multiple --detail parameter values', async () => {
    mockFetch.mockImplementation(async (url) => {
      if (url.startsWith('https://api.anthropic.com')) {
        return {
          ok: true,
    ...
    const result = await runScript(
      ['https://example.com/image.jpg', '--detail', 'low'],
      { ANTHROPIC_API_KEY: 'test-key' }
    );
  ...
  it('handles BMP file format', async () => {
    const tempFile = join(tmpdir(), `test-image-${Date.now()}.bmp`);
    tempFiles.push(tempFile);
    writeFileSync(tempFile, Buffer.from('fake-bmp-data'));

    mockFetch.mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: 'BMP image analysis.' }],
        usage: { input_tokens: 1500, output_tokens: 20 },
      }),
    });
  ...
  it('handles case-insensitive file extensions', async () => {
    ...
    mockFetch.mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: 'PNG image analysis.' }],
        usage: { input_tokens: 1500, output_tokens: 20 },
      }),
    });
