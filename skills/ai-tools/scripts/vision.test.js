...
describe('vision.js', () => {
  ...
  it('handles very large images', async () => {
    ...
    writeFileSync(tempFile, largeBuffer);
    ...
    mockFetch.mockImplementation(async (url) => {
      if (url.startsWith('https://api.anthropic.com')) {
        return {
          ok: true,
          json: async () => ({
            content: [{ text: 'Large image analyzed.' }],
  ...
  it('handles image URL with redirect', async () => {
    mockFetch.mockImplementation(async (url) => {
      if (url.startsWith('https://api.anthropic.com')) {
        return {
          ok: true,
          json: async () => ({
            content: [{ text: 'Redirected image.' }],
            usage: { input_tokens: 1500, output_tokens: 20 },
          }),
        };
      }
    ...
    const result = await runScript(['https://example.com/redirect-image.png'], {
      ANTHROPIC_API_KEY: 'test-key',
    });
  ...
  it('handles complex prompts with special characters', async () => {
    mockFetch.mockImplementation(async (url) => {
      if (url.startsWith('https://api.anthropic.com')) {
        return {
          ok: true,
    ...
    const result = await runScript(
      ['https://example.com/image.jpg', 'What\'s this? @#$% émojis 🎉'],
      { ANTHROPIC_API_KEY: 'test-key' }
    );
  ...
  it('handles .jpeg extension (alternate JPEG extension)', async () => {
  ...
  it('handles GIF images', async () => {
    ...
    tempFiles.push(tempFile);
    writeFileSync(tempFile, Buffer.from('fake-gif-data'));

    mockFetch.mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: 'GIF image (treated as PNG).' }],
        usage: { input_tokens: 1500, output_tokens: 20 },
      }),
    });
  ...
  it('handles URL with no content-type header', async () => {
    mockFetch.mockImplementation(async (url) => {
      if (url.startsWith('https://api.anthropic.com')) {
        return {
          ok: true,
  ...
  it('handles empty prompt with default', async () => {
    mockFetch.mockImplementation(async (url) => {
      if (url.startsWith('https://api.anthropic.com')) {
        return {
          ok: true,
          json: async () => ({
            content: [{ text: 'Default prompt used.' }],
            usage: { input_tokens: 1500, output_tokens: 20 },
          }),
        };
      }
  ...
  it('handles API 413 payload too large error', async () => {
    mockFetch.mockImplementation(async (url) => {
      if (url.startsWith('https://api.anthropic.com')) {
        return {
          ok: false,
  ...
  it('handles file path with spaces', async () => {
    ...
    mockFetch.mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: 'Image with spaces in path.' }],
        usage: { input_tokens: 1500, output_tokens: 20 },
      }),
    });
  ...
  it('handles URL with port number', async () => {
    mockFetch.mockImplementation(async (url) => {
      if (url.startsWith('https://api.anthropic.com')) {
        return {
          ok: true,
          json: async () => ({
            content: [{ text: 'Image from URL with port.' }],
            usage: { input_tokens: 1500, output_tokens: 20 },
          }),
        };
      }
    ...
    const result = await runScript(['https://example.com:8080/image.jpg'], {
      ANTHROPIC_API_KEY: 'test-key',
    });
  ...
  it('validates fetch is called with correct anthropic parameters', async () => {
    mockFetch.mockImplementation(async (url) => {
      if (url.startsWith('https://api.anthropic.com')) {
        return {
          ok: true,
    ...
    await runScript(['https://example.com/image.jpg', 'What is this?'], {
      ANTHROPIC_API_KEY: 'test-key',
    });

    const anthropicCalls = mockFetch.mock.calls.filter(call =>
      call[0].startsWith('https://api.anthropic.com')
    );
    expect(anthropicCalls.length).toBeGreaterThan(0);
    expect(anthropicCalls[0][1]).toMatchObject({
      method: 'POST',
