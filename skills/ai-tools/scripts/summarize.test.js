...
describe('summarize.js', () => {
  ...
  it('handles zero length parameter', async () => {
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: '' }],
  ...
  it('handles negative length parameter', async () => {
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: 'Summary.' }],
    ...
    const result = await runScript(['text', '--length', '-50'], {
      ANTHROPIC_API_KEY: 'test-key',
    });
  ...
  it('handles file with no extension', async () => {
    const tempFile = join(tmpdir(), `test-summarize-noext-${Date.now()}`);
    tempFiles.push(tempFile);
    ...
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: 'Summary of file content.' }],
  ...
  it('handles Unicode characters in text', async () => {
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: 'Summary with Chinese and emojis' }],
        usage: { input_tokens: 20, output_tokens: 15 },
      }),
    });
    global.fetch = mockFetch;

    const result = await runScript(['Text with Chinese characters and emojis'], {
      ANTHROPIC_API_KEY: 'test-key',
    });
  ...
  it('handles summary returning exactly target length', async () => {
    ...
    const result = await runScript(['long text', '--length', '100'], {
      ANTHROPIC_API_KEY: 'test-key',
    });

    expect(result.code).toBe(0);
    const output = JSON.parse(result.stdout);
    expect(output.actualWords).toBe(100);
    expect(output.targetWords).toBe(100);
  });

  it('handles very long file content', async () => {
    const tempFile = join(tmpdir(), `test-summarize-long-${Date.now()}.txt`);
    tempFiles.push(tempFile);
    ...
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: 'Brief summary of very long file.' }],
        usage: { input_tokens: 10000, output_tokens: 20 },
      }),
    });
    global.fetch = mockFetch;
  ...
  it('handles summary with newlines and formatting', async () => {
    const summaryWithFormatting = 'Line 1\\n\\nLine 2\\n\\n- Bullet\\n- Another bullet';
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: summaryWithFormatting }],
  ...
  it('validates word count calculation with multiple spaces', async () => {
    const summary = 'word  double   triple    spaces';
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: summary }],
