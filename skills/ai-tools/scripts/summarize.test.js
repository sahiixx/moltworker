...
describe('summarize.js', () => {
  ...
  it('handles zero target word length', async () => {
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: 'Summary.' }],
  ...
  it('handles summary longer than original text', async () => {
    const shortText = 'Hi';
    const longSummary = 'This is a very detailed and comprehensive summary that is much longer than the original text.';
    ...
    const result = await runScript([shortText], {
      ANTHROPIC_API_KEY: 'test-key',
    });

    expect(result.code).toBe(0);
    const output = JSON.parse(result.stdout);
    expect(output.actualWords).toBeGreaterThan(output.originalLength);
  });

  it('handles summary with multiple newlines', async () => {
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: 'Line 1\n\nLine 2\n\n\nLine 3' }],
  ...
  it('handles API response with missing content', async () => {
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [],
  ...
  it('handles bullets style with numbered list', async () => {
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: '1. First point\n2. Second point\n3. Third point' }],
        usage: { input_tokens: 100, output_tokens: 30 },
      }),
    });
    global.fetch = mockFetch;
