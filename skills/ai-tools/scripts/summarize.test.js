...
describe('summarize.js', () => {
  ...
  it('handles empty file content', async () => {
    const tempFile = join(tmpdir(), `test-empty-${Date.now()}.txt`);
    tempFiles.push(tempFile);
  ...
  it('handles negative length value', async () => {
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: 'Summary.' }],
  ...
  it('handles zero length value', async () => {
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: '' }],
  ...
  it('handles API 400 bad request error', async () => {
    const mockFetch = vi.fn().mockResolvedValue({
      ok: false,
      status: 400,
  ...
  it('handles summary with only whitespace', async () => {
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: '   \n\t  ' }],
  ...
  it('handles file with binary content', async () => {
    const tempFile = join(tmpdir(), `test-binary-${Date.now()}.bin`);
    tempFiles.push(tempFile);
    ...
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: 'Binary content summary.' }],
        usage: { input_tokens: 10, output_tokens: 10 },
      }),
    });
    global.fetch = mockFetch;
  ...
  it('handles combination of all options', async () => {
    ...
    const result = await runScript(
      ['text', '--length', '150', '--style', 'bullets', '--model', 'claude-3-opus-20240229'],
      { ANTHROPIC_API_KEY: 'test-key' }
    );

    expect(result.code).toBe(0);
    const output = JSON.parse(result.stdout);
    expect(output.targetWords).toBe(150);
    expect(output.style).toBe('bullets');
    expect(output.model).toBe('claude-3-opus-20240229');
  });
  ...
  it('validates fetch is called with correct parameters', async () => {
    ...
    await runScript(['test text', '--length', '200', '--style', 'detailed'], {
      ANTHROPIC_API_KEY: 'test-key',
    });
  ...
  it('handles text with tabs and mixed whitespace', async () => {
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        content: [{ text: 'Summary of mixed whitespace text.' }],
        usage: { input_tokens: 50, output_tokens: 15 },
      }),
    });
    global.fetch = mockFetch;

    const result = await runScript(['text\twith\t\ttabs  and   spaces'], {
      ANTHROPIC_API_KEY: 'test-key',
    });
