...
describe('embeddings.js', () => {
  ...
  it('saves embeddings to file with --output parameter', async () => {
    const fs = require('fs');
    const outputFile = '/tmp/test-embeddings.json';
    ...
    if (fs.existsSync(outputFile)) {
      ...
      expect(content).toHaveProperty('embedding');
      expect(content).toHaveProperty('dimensions');
  ...
  it('handles very long input text', async () => {
    ...
    expect(result.code).toBe(0);
    const output = JSON.parse(result.stdout);
    expect(output.usage.tokens).toBe(10000);
  });

  it('handles zero-dimension embeddings', async () => {
    const mockResponse = {
      ok: true,
      json: async () => ({
        data: [{ embedding: [] }],
  ...
  it('handles various dimension sizes', async () => {
    const dimensions = [256, 512, 1024, 3072];

    for (const dim of dimensions) {
      const mockResponse = {
        ok: true,
        json: async () => ({
          data: [{ embedding: Array(dim).fill(0.1) }],
          usage: { total_tokens: 5 },
        }),
      };
      ...
      const result = await runScript(['test', '--dimensions', dim.toString()], {
        OPENAI_API_KEY: 'test-key',
      });

      expect(result.code).toBe(0);
      const output = JSON.parse(result.stdout);
      expect(output.dimensions).toBe(dim);
    }
  });
});