...
describe('embeddings.js', () => {
  ...
  it('handles non-numeric dimensions parameter', async () => {
    const mockResponse = {
      ok: true,
      json: async () => ({
        data: [{ embedding: [0.1, 0.2] }],
    ...
    const result = await runScript(['test text', '--dimensions', 'not-a-number'], {
      OPENAI_API_KEY: 'test-key',
    });
  ...
  it('handles very large dimensions value', async () => {
    const largeEmbedding = Array(10000).fill(0.1);
    const mockResponse = {
      ok: true,
      json: async () => ({
    ...
    const result = await runScript(['test text', '--dimensions', '10000'], {
      OPENAI_API_KEY: 'test-key',
    });

    expect(result.code).toBe(0);
    const output = JSON.parse(result.stdout);
    expect(output.embedding).toHaveLength(7);
    expect(output.embedding[6]).toContain('10000 total');
  });
