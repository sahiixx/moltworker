...
describe('embeddings.js', () => {
  ...
  it('handles negative dimensions parameter', async () => {
    ...
    const result = await runScript(['test text', '--dimensions', '-100'], {
      OPENAI_API_KEY: 'test-key',
    });
  ...
  it('handles API returning empty embedding array', async () => {
    const mockResponse = {
      ok: true,
      json: async () => ({
        data: [{ embedding: [] }],
    ...
    expect(result.code).toBe(0);
    const output = JSON.parse(result.stdout);
    // Even empty arrays are displayed with truncation format
    expect(output.embedding).toEqual(['...', '(0 total)']);
  });
  ...
  it('handles concurrent requests with same API key', async () => {
    const mockResponse = {
      ok: true,
      json: async () => ({
        data: [{ embedding: [0.1, 0.2] }],
  ...
  it('handles output flag for saving results', async () => {
    const mockResponse = {
      ok: true,
      json: async () => ({
        data: [{ embedding: [0.1, 0.2, 0.3] }],
    ...
    const outputPath = '/tmp/test-embeddings-output.json';
    const result = await runScript(['test text', '--output', outputPath], {
      OPENAI_API_KEY: 'test-key',
    });
    ...
    expect(output.saved).toBe(outputPath);
  });

  it('handles Unicode and emoji in input text', async () => {
    const mockResponse = {
      ok: true,
      json: async () => ({
        data: [{ embedding: [0.1, 0.2] }],
    ...
    const result = await runScript(['Hello 世界 🌍 emoji'], {
      OPENAI_API_KEY: 'test-key',
    });
  ...
  it('validates embedding dimensions match requested', async () => {
    const mockResponse = {
      ok: true,
      json: async () => ({
    ...
    const result = await runScript(['test text', '--dimensions', '256'], {
      OPENAI_API_KEY: 'test-key',
    });

    expect(result.code).toBe(0);
    const output = JSON.parse(result.stdout);
    expect(output.dimensions).toBe(256);
  });
  ...
  it('handles 403 Forbidden error', async () => {
    ...
    expect(result.code).toBe(1);
    const error = JSON.parse(result.stderr);
    expect(error.error).toContain('403');
  });
