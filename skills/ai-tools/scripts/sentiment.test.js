...
describe('sentiment.js', () => {
  ...
  it('handles sentiment with zero confidence', async () => {
    const mockAnalysis = {
      sentiment: 'neutral',
      score: 0.0,
      confidence: 0.0,
    ...
    const result = await runScript(['unclear text'], {
      ANTHROPIC_API_KEY: 'test-key',
    });

    expect(result.code).toBe(0);
    const output = JSON.parse(result.stdout);
    expect(output.analysis.confidence).toBe(0.0);
  });

  it('handles sentiment with no emotions detected', async () => {
    const mockAnalysis = {
      sentiment: 'neutral',
      score: 0.0,
      confidence: 0.6,
  ...
  it('handles sentiment with very long keyword list', async () => {
    const longKeywords = Array(100).fill('word');
    const mockAnalysis = {
      sentiment: 'positive',
      score: 0.7,
      confidence: 0.8,
    ...
    const result = await runScript(['text with many keywords'], {
      ANTHROPIC_API_KEY: 'test-key',
    });

    expect(result.code).toBe(0);
    const output = JSON.parse(result.stdout);
    expect(output.analysis.keywords.length).toBe(100);
  });
  ...
  it('handles sentiment with fractional scores', async () => {
    const mockAnalysis = {
      sentiment: 'positive',
      score: 0.123456789,
      confidence: 0.987654321,
      emotions: [{ emotion: 'joy', intensity: 0.555555 }],
      tone: 'optimistic',
      keywords: ['good'],
    };
    ...
    expect(result.code).toBe(0);
    const output = JSON.parse(result.stdout);
    expect(output.analysis.score).toBeCloseTo(0.123456789);
  });
  ...
  it('handles sentiment with Unicode emotions', async () => {
    const mockAnalysis = {
      sentiment: 'positive',
      score: 0.8,
      confidence: 0.9,
      emotions: [{ emotion: '😊 happiness', intensity: 0.8 }],
      tone: 'cheerful',
      keywords: ['great', '✨'],
    };
    ...
    const result = await runScript(['Great day! ✨'], {
      ANTHROPIC_API_KEY: 'test-key',
    });

    expect(result.code).toBe(0);
    const output = JSON.parse(result.stdout);
    expect(output.analysis.emotions[0].emotion).toContain('😊');
  });

  it('handles sentiment analysis of numbers', async () => {
    const mockAnalysis = {
      sentiment: 'neutral',
      score: 0.0,
      confidence: 0.5,
    ...
    const result = await runScript(['123 456 789'], {
      ANTHROPIC_API_KEY: 'test-key',
    });
  ...
  it('validates usage tracking for high token input', async () => {
    const mockAnalysis = {
      sentiment: 'positive',
      score: 0.6,
      confidence: 0.8,
    ...
    const result = await runScript([('word '.repeat(5000))], {
      ANTHROPIC_API_KEY: 'test-key',
    });

    expect(result.code).toBe(0);
    const output = JSON.parse(result.stdout);
    expect(output.usage.input_tokens).toBe(10000);
  });

  it('handles sentiment with boundary score values', async () => {
    const mockAnalysis = {
      sentiment: 'positive',
      score: 1.0,
      confidence: 1.0,
      emotions: [{ emotion: 'ecstasy', intensity: 1.0 }],
      tone: 'extreme',
      keywords: ['best', 'perfect'],
    };
    ...
    const result = await runScript(['absolutely perfect'], {
      ANTHROPIC_API_KEY: 'test-key',
    });
    ...
    const output = JSON.parse(result.stdout);
    expect(output.analysis.score).toBe(1.0);
    expect(output.analysis.confidence).toBe(1.0);
  });
});